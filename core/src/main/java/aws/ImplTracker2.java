...
public  class ImplTracker2 implements Tracker {
    Comparator<Order> comparator = (a,b) -> a.time.compareTo(b.time);
    TreeMap<Order, Integer> orders = new TreeMap(comparator);

    @Override
    public Integer addOrder(int costomerId, Order order){
        return orders.put( order,  costomerId);
    }
    ...
    public List<Order> getRange(DateTime from, DateTime to){
        ...
        orTo.time = to;
        comparator = (a,b) -> (-1)*a.time.compareTo(b.time);
        orderList.addAll(orders.subMap(orFrom,orTo).keySet());
        return orderList;
...
//    }
    ...
    public static void main(String[] args) {
        ...
        tracker.getRange(new DateTime(1983,1,1,1,1), new DateTime(1989,1,1,1,1))
                .forEach( order ->
