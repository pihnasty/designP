 ...
 * the editor.
 */
package Mash.task_03_ExecutorService;
        ...
        btnS.setOnAction(new EventHandler<ActionEvent>() {
            ...
            public void handle(ActionEvent e) {
                final Task task = new Task<Void>() {
                    ...
                    protected void updateProgress2(long workDone, long max) {
                        pb.setProgress((double) workDone / (double) max/2);
                    }
                    @Override
                ...
                es.submit(task);
                // es.submit(task).get();   // Получаем значение возвращаемое методом  Task.call();
                es.shutdown();
